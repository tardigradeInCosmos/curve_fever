...
fn board_build_with_terminal_size_accessible() {
    let mut tsa_mock = MockTerminalSizeAcquisitor::new();
    tsa_mock.expect_get().return_const(Some((Width(11), Height(31))));
    let b = Board::build_with(tsa_mock);
    assert_eq!(b.w, 11u16, "board width value expected to match 11, got {}", b.w);
    assert_eq!(b.h, 31u16, "board height value expected to match 31, got {}", b.h);
}
