use terminal::MockTerminalSizeAcquisitor;
use crate::view::board::terminal::{Height, Width};
use super::*;
...
fn board_build_with_terminal_size_inaccesible() {
    ...
    let b = Board::build_with(tsa_mock).unwrap();
    assert_eq!(
        b.w, 10u16,
...
fn board_create_printable_0() {
    let b = Board::new((Width(4), Height(5)));
...
fn board_create_printable_1() {
    let b = Board::new((Width(10), Height(7)));
    let result = b.create_printable();
    let expected = "\
__________
|        |
